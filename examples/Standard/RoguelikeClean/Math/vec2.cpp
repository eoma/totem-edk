...
Vec2<Type>::Vec2(const Pointx<int> &point) { x = (Type) point.x; y = (Type) point.y; }

template<typename Type>
Vec2<Type>::Vec2(const Pointx<float> &point) { x = (Type) point.x; y = (Type) point.y; }

template<typename Type>
Vec2<Type>::Vec2(const Pointx<double> &point) { x = (Type) point.x; y = (Type) point.y; }

// Add some rounding specializations

template<>
Vec2<int>::Vec2(const Pointx<int> &point) { x = point.x ; y = point.y; }

template<>
Vec2<int>::Vec2(const Pointx<float> &point) { x = (int) floor(point.x + 0.5f); y = (int) floor(point.y + 0.5f); }

template<>
Vec2<int>::Vec2(const Pointx<double> &point) { x = (int) floor(point.x + 0.5f); y = (int) floor(point.y + 0.5); }

template<>
Vec2<unsigned int>::Vec2(const Pointx<int> &point) { x = (unsigned int) point.x ; y = (unsigned int) point.y; }

template<>
Vec2<unsigned int>::Vec2(const Pointx<float> &point) { x = (unsigned int) floor(point.x + 0.5f); y = (unsigned int) floor(point.y + 0.5f); }

template<>
Vec2<unsigned int>::Vec2(const Pointx<double> &point) { x = (unsigned int) floor(point.x + 0.5f); y = (unsigned int) floor(point.y + 0.5); }

template<>
Vec2<short>::Vec2(const Pointx<int> &point) { x = point.x ; y = point.y; }

template<>
Vec2<short>::Vec2(const Pointx<float> &point) { x = (short) floor(point.x + 0.5f); y = (short) floor(point.y + 0.5f); }

template<>
Vec2<short>::Vec2(const Pointx<double> &point) { x = (short) floor(point.x + 0.5f); y = (short) floor(point.y + 0.5); }

template<>
Vec2<unsigned short>::Vec2(const Pointx<int> &point) { x = (unsigned short) point.x ; y = (unsigned short) point.y; }

template<>
Vec2<unsigned short>::Vec2(const Pointx<float> &point) { x = (unsigned short) floor(point.x + 0.5f); y = (unsigned short) floor(point.y + 0.5f); }

template<>
Vec2<unsigned short>::Vec2(const Pointx<double> &point) { x = (unsigned short) floor(point.x + 0.5f); y = (unsigned short) floor(point.y + 0.5); }

template<>
Vec2<char>::Vec2(const Pointx<int> &point) { x = point.x ; y = point.y; }

template<>
Vec2<char>::Vec2(const Pointx<float> &point) { x = (char) floor(point.x + 0.5f); y = (char) floor(point.y + 0.5f); }

template<>
Vec2<char>::Vec2(const Pointx<double> &point) { x = (char) floor(point.x + 0.5f); y = (char) floor(point.y + 0.5); }

template<>
Vec2<unsigned char>::Vec2(const Pointx<int> &point) { x = (unsigned char) point.x ; y = (unsigned char) point.y; }

template<>
Vec2<unsigned char>::Vec2(const Pointx<float> &point) { x = (unsigned char) floor(point.x + 0.5f); y = (unsigned char) floor(point.y + 0.5f); }

template<>
Vec2<unsigned char>::Vec2(const Pointx<double> &point) { x = (unsigned char) floor(point.x + 0.5f); y = (unsigned char) floor(point.y + 0.5); }

template<typename Type>
Type Vec2<Type>::length() const {return (Type) floor(sqrt(float(x*x+y*y))+0.5f);}

template<>
double Vec2<double>::length() const {return sqrt(x*x+y*y);}

template<>
float Vec2<float>::length() const {return sqrt(x*x+y*y);}
...
template<typename Type>
Vec2<Type> Vec2<Type>::normalize(const Vec2<Type>& vector)
{
