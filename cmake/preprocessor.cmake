...
macro(define_totem_paths package)
	...
	set(TOTEM_HDRS_EXT ${TotemEDK_SOURCE_DIR}/include/Totem/Extensions)
	set(TOTEM_CONFIG_HDRS ${TotemEDK_SOURCE_DIR}/depends/${package})
	
	message(STATUS "Path to Totem include/Totem/: ${TOTEM_HDRS}")
	message(STATUS "Path to Totem include/Totem/Extensions/: ${TOTEM_HDRS_EXT}")
...
macro(extract_files package)
	...
	file(GLOB totem_hdr_ext_files_abs_path ${TOTEM_HDRS_EXT}/*.h ${TOTEM_HDRS_EXT}/*.inl)
	...
	message(STATUS "Header Files in include/Totem/Extensions/ ${totem_hdr_ext_files_abs_path}")
...
macro(define_output_paths package)
	...
	set(PACKAGE_HDRS_EXT_DIR ${PACKAGE_OUTPUT_DIR}/include/Totem/Extensions)
	...
	message(STATUS "Output Path to Totem ${package} include/Totem/Extensions/: ${PACKAGE_HDRS_EXT_DIR}")
...
macro(split_into_filenames package)
	...
	ENDFOREACH(f ${ARGN})
	message(STATUS " ")
	message(STATUS "include/Totem/Extensions/ filenames:")
	FOREACH(f ${totem_hdr_ext_files_abs_path})
		FILE(RELATIVE_PATH folder ${TOTEM_HDRS_EXT} ${f})
		get_filename_component(filename ${folder} NAME)
		set(totem_hdr_ext_files ${totem_hdr_ext_files} ${filename})
		message(STATUS "- ${filename}")
	ENDFOREACH(f ${ARGN})
	message(STATUS " ")
...
macro(define_output_abs_paths package)
	...
	set(package_hdr_ext_files)
	set(package_config_hdr_files)
	
	foreach(f ${totem_hdr_files})
		set(package_hdr_files ${package_hdr_files} ${PACKAGE_HDRS_DIR}/${f})
	endforeach()
	foreach(f ${totem_hdr_ext_files})
		set(package_hdr_ext_files ${package_hdr_ext_files} ${PACKAGE_HDRS_EXT_DIR}/${f})
	endforeach()
	...
	message(STATUS "Header Files to editions/${package}/include/Totem/Extensions/ ${package_hdr_ext_files}")
...
macro(copy_totem_to_output package)
	...
	set(list_count)
	set(list_index 0)
	list(LENGTH package_hdr_ext_files list_count)
	foreach(input ${totem_hdr_ext_files_abs_path})
		list(GET package_hdr_ext_files ${list_index} output)
		message(STATUS "Evaluating index ${list_index}/${list_count}: ${output}")
		MATH(EXPR list_index "${list_index}+1")
		
		configure_file(${input} ${output} COPYONLY)
	endforeach(input ${totem_hdr_ext_files_abs_path})
	message(STATUS " ")
	...
	set(output_files ${package_hdr_files} ${package_hdr_ext_files})
